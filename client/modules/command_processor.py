...
PERMISSION_LEVEL_VALUES = {
    'player': 0,
    'vip': 1, #TODO: VIP state needs to be implemented
    'support': 1,
    'moderator': 2,
    'admin': 3,
    'cftools_staff': 4
...
class ChatCommands(object):
    ...
    def register_callbacks(self):
        self.worker.register_callback('player', 'chat', self.incoming_chat)
    ...
    def _process_command(self, player, command, params):
        ...
        if command.get('action') == 'send_basic_message':
            ...
            if len(params) > 0:
                ...
                search_result = self.worker.find_player_by_name(player_name)
        ...
        elif command.get('action') == 'send_custom_message':
            message = ' '.join(params)
            self.worker.server.say_all(message)
